...
## 正式发版 2.2.1（2026-07-02）

仓库根目录 `pyproject.toml` 发布 **2.2.1**（自最新 public bundle 刷新快照）。
- 安装: `pip install manastone-diag==2.2.1` 或 `pip install manastone-diag`
- 验证: `curl -s https://pypi.org/pypi/manastone-diag/json` 应包含 `2.2.1`

## 正式发版 2.2.0（2026-07-02）

仓库根目录 `pyproject.toml` 已作为 **正式 PyPI 包** 发布 **2.2.0**，替代此前占名的 **0.0.1** stub。
- 安装: `pip install manastone-diag==2.2.0`
- 验证: `curl -s https://pypi.org/pypi/manastone-diag/json` 应包含 `2.2.0`
- 本目录 `packaging/pypi-stub/` 仍保留 0.0.1 占名包源码备份，勿再上传 stub 覆盖正式版。
