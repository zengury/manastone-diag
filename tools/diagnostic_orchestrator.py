...
用法 (被 LLM 通过 pi 调用):
    # 第一轮前: 初始化 session
    python -m tools.diagnostic_orchestrator init --session X220028C4Z0079

    # 每轮开始: 检查门禁 + 获取检查清单
    python -m tools.diagnostic_orchestrator round 1 --session X220028C4Z0079

    # 每轮结束: 验证产出 + 写入 checkpoint
    python -m tools.diagnostic_orchestrator validate 1 --session X220028C4Z0079

    # 查看状态
    python -m tools.diagnostic_orchestrator status --session X220028C4Z0079
"""
